...
public class FuncUtil {
    ...
    private static Field[] getAllDeclaredFields(Class c) {
        ...
        if ((parentFields != null) && (parentFields.length > 0)) {
            Field[] allFields = new Field[parentFields.length + thisFields.length];
            System.arraycopy(parentFields, 0, allFields, 0, parentFields.length);
            System.arraycopy(thisFields, 0, allFields, parentFields.length, thisFields.length);
    ...
    public static boolean validSign(TreeMap<String, String> param, String appkey) throws Exception {
        if (param != null && !param.isEmpty()) {
            ...
            return sign.toLowerCase().equals(md5(blank.getBytes(StandardCharsets.UTF_8)));
        }
        return true;
    }
